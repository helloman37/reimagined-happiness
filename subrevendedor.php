<?php
	...
	if (checarUsuario()) {
		...
		$listas = listarListas();
		$buscar = ("SELECT * FROM usuario WHERE id_criador = ".$_SESSION['id_usuario']." AND vendedor = 1 and admin = 0");
				...
				<div id="conteudo-painel" style="padding-left: 0px;" class="table-responsive container">
						...
						<div class="col-12 col-lg-12 col-xl-12" style="padding-left:0px; padding-right:0px">
							<div class="form-row">
								...
								<?php if($_SESSION['vendedor']) { ?>
									<?php /*if($creditos <= $credito){*/ if($credito>0){ ?>
										<div class="col col-md-2">
											<i onclick="$('#cadastro').modal()" class="btn btn-outline-info text-dark fas fa-user-plus" style="line-height:1.45;float:right"></i>
										</div>
									<?php } ?>
					...
					<table class='table table-bordered table-hover table-responsive'>
						<thead class="thead-light">
							<tr align="center">
                                <th class='nomecol' style="width: 10%" scope="col">Sub-Revendedor</th>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="TituloModalLongoExemplo">Adicionar Sub-Revendedor</h5>
...
<?php if($_SESSION['vendedor']) { ?>
    ...
    <div class="modal fade" id="editar" tabindex="-1" role="dialog" aria-labelledby="Cadastrar" aria-hidden="true" data-backdrop="static" data-keyboard="false">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Editar Sub-Revendedor</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Fechar">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
    ...
    <div class="modal fade" id="alerta" tabindex="-1" role="dialog" aria-labelledby="Cadastrar" aria-hidden="true" data-backdrop="static" data-keyboard="false">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header" style="background-color:#ffeeba">
                    <h5 class="modal-title" style="color:red">AVISO!</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Fechar">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div id="alerta-conteudo" class="modal-body"></div>
                <div class="modal-footer" style="background-color:#ffeeba; padding:10px;">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Fechar</button>
                </div>                
            </div>
        </div>
...
<script>
	...
	function remover(id){
        window.location.href = 'controles/remover-subrevendedor.php?id_usuario=' + id;
    }
    ...
    $( "#cadastro-form" ).submit(function( event ) {
        if($('#creditoC').val() > <?php echo $credito; ?>){
            $('#alerta-conteudo').html('<div class="alert alert-danger" style="margin-bottom:0;" role="alert"><strong> Nº de créditos ultrapassa o seu limite de '+<?php echo $credito; ?>+' credito(s)!</strong></div>');
            $('#alerta').modal();
            event.preventDefault();
        }else{
            $.ajax({
                type: "POST",
                url: "controles/adicionar-subrevendedor.php",
    ...
    $( "#editar-form" ).submit(function( event ) {
        if($('#creditoE').val() > <?php echo $credito; ?>){
            $('#alerta-conteudo').html('<div class="alert alert-danger" style="margin-bottom:0;" role="alert"><strong> Nº de créditos ultrapassa o seu limite de '+<?php echo $credito; ?>+' credito(s)!</strong></div>');
            $('#alerta').modal();
            event.preventDefault();
        }else{        
            $.ajax({
                type: "POST",
                url: "controles/editar-subrevendedor.php",
